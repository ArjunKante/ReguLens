...
const GradientWaves = ({
  ...
  // WebGL2 isn't guaranteed on every officer's workstation. Rather than let
  // a construction failure leave a blank/broken hero (this page still has
  // to be usable to sign in), fall back to a plain CSS gradient — set via
  // this flag and a CSS class rather than letting an exception propagate.
  ...
  return (
    <div
      ref={containerRef}
      className={`gradient-waves-container ${!supported ? "gradient-waves-container--fallback" : ""} ${className}`.trim()}
    />
  );
};
